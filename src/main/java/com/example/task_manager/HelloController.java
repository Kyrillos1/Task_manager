...
public class HelloController {
    ...
    @FXML
    private ComboBox<Task.Status2> Show;
    @FXML
    ...
    @FXML
    void onShowButtonAction(ActionEvent event){
        Task.Status2 status = Show.getValue();
        ObservableList<Task> observelistTasks = FXCollections.observableArrayList(tasks);

        if (status != null && !status.toString().equals("ALL") ){
            FilteredList<Task> filteredData = new FilteredList<>(observelistTasks, p -> true);
            filteredData.setPredicate(task -> task.getStatus().toString().equals(status.toString()));
    ...
    public void initialize() {
        ...
        ObservableList<Task.Status2> statusOptions2 = FXCollections.observableArrayList(Task.Status2.values());
        Show.setItems(statusOptions2);
